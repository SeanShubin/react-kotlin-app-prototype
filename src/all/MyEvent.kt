...
class EventLoopImpl : EventLoop {
    override fun reactTo(state: MyState, event: MyEvent): StateAndEffects {
        ...
        val result: StateAndEffects = when (event) {
            ...
            is UpdateFooRequest -> default.copy(state = FooState(event.newValue))
            is UpdateBarRequest -> default.copy(state = BarState(event.newValue))
            is LoadFooResponse -> default.copy(state = FooState(event.newValue))
            is LoadBarResponse -> default.copy(state = BarState(event.newValue))
            else -> default.copy(state = UnsupportedEventState(event.toString()))
        }
